...
class ReturnVoidSniff implements Sniff
{
        ...
        foreach ($annotations as $annotationName => $annotationsByName) {
            ...
            foreach ($annotationsByName as $annotation) {
                if ($annotationName === '@return') {
                    ...
                    if (strcasecmp($content, 'void') === 0) {
                        $fix = $phpcsFile->addFixableError(sprintf('Use of "@return void" is forbidden.'), $annotation->getStartPointer(), self::CODE_ANNOTATION_FORBIDDEN);
                        if (!$fix) {
                            continue;
                        }
                    }
    ...
    /**
     * Copied almost verbatim from ForbiddenAnnotationsSniff (Slevomat)
     *
     * @param File $phpcsFile
     * @param $docCommentOpenPointer
     * @param Annotation $annotation
     * @see ForbiddenAnnotationsSniff
     */
    private function fix(File $phpcsFile, $docCommentOpenPointer, Annotation $annotation)
    {
